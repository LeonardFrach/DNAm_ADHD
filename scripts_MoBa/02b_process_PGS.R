...
setwd("/cluster/projects/p471/people/Leo/ADHD/")
...
processed_prsRes <- processed_prs %>% 
  dplyr::mutate_at(vars(tidyr::matches("PGS_")), list(res = ~model(.))) %>% 
  dplyr::select(IID, FID, tidyr::matches("PREG_ID"), BARN_NR, 
                tidyr::matches("F_ID"),
                tidyr::matches("M_ID"), Role, 
                tidyr::matches("PGS_"),
                tidyr::matches("PC"), 
                genotyping_batch, Plate_id, imputation_batch)
